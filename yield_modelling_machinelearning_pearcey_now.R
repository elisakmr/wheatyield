...
myFiles <- list()
myFiles[["calibration"]] <- list.files(path=file.path(dir_data, "Calibration"), pattern="calibrationnow_",full.names=TRUE) # gather all the calibration data
myFiles[["validation"]] <- list.files(path=file.path(dir_data, "Validation"), pattern="validationnow_",full.names=TRUE) # gather all the validation data
...
for (crop_i in crop_list){ # crop_i = calibration data frames per crop
  ### Load cal/val data ---------------
  print(crop_i)
  load(grep(paste0("calibrationnow_", crop_i, ".RData"), myFiles$calibration, value = T))
  load(grep(paste0("validationnow_", crop_i, ".RData"), myFiles$validation, value = T))
  
  
  ### load learners -------------------
  load(file.path(dir_mdl, paste0("rf_optimised_now_",crop_i, ".RData" )))
  load(file.path(dir_mdl, paste0("cub_optimised_now_",crop_i, ".RData" )))
  load(file.path(dir_mdl, paste0("xb_optimised_now_",crop_i, ".RData" )))
  load(file.path(dir_mdl, paste0("svm_optimised_now_",crop_i, ".RData" )))
  load(file.path(dir_mdl, paste0("svmr_optimised_now_",crop_i, ".RData" )))
  load(file.path(dir_mdl, paste0("nn_optimised_now_",crop_i, ".RData" )))
  load(file.path(dir_mdl, paste0("gp_optimised_now_",crop_i, ".RData" )))
  load(file.path(dir_mdl, paste0("ea_optimised_now_",crop_i, ".RData" )))
  load(file.path(dir_mdl, paste0("knn_optimised_now_",crop_i, ".RData" )))
  ...
  # SD calculation
  print("Get SD")
  sd_rf <- getSDPrediction(select(df_cal_i,starts_with("NDVI")), df_cal_i$yield, select(df_val_i,starts_with("NDVI")), 
                           method = "rf", grid = rf_gridsearch$bestTune, pp = c('center', 'scale'),  k = 25)
  
  sd_cub <- getSDPrediction(select(df_cal_i,starts_with("NDVI")), df_cal_i$yield, select(df_val_i,starts_with("NDVI")), 
                            method = "cubist", grid = cub_gridsearch$bestTune, pp = c('center', 'scale'),  k = 25)
  
  sd_xb <- getSDPrediction(select(df_cal_i,starts_with("NDVI")), df_cal_i$yield, select(df_val_i,starts_with("NDVI")), 
                           method = "xgbTree", grid = xb_gridsearch$bestTune, pp = c('center', 'scale'),  k = 25)
  
  sd_svm <- getSDPrediction(select(df_cal_i,starts_with("NDVI")), df_cal_i$yield, select(df_val_i,starts_with("NDVI")), 
                            method = "svmLinear", grid = svm_gridsearch$bestTune, pp = c('center', 'scale'),  k = 25)
  
  sd_svmr <- getSDPrediction(select(df_cal_i,starts_with("NDVI")), df_cal_i$yield, select(df_val_i,starts_with("NDVI")), 
                             method = "svmRadial", grid = svmr_gridsearch$bestTune, pp = c('center', 'scale'),  k = 25)
  
  sd_nn <- getSDPrediction(select(df_cal_i,starts_with("NDVI")), df_cal_i$yield, select(df_val_i,starts_with("NDVI")), 
                           method = "mlpML", grid = nn_gridsearch$bestTune, pp = c('center', 'scale'),  k = 25)
  
  sd_gp <- getSDPrediction(select(df_cal_i,starts_with("NDVI")), df_cal_i$yield, select(df_val_i,starts_with("NDVI")), 
                           method = "gaussprRadial", grid = gp_gridsearch$bestTune, pp = c('center', 'scale'),  k = 25)
  
  sd_ea <- getSDPrediction(select(df_cal_i,starts_with("NDVI")), df_cal_i$yield, select(df_val_i,starts_with("NDVI")), 
                           method = "earth", grid = ea_gridsearch$bestTune, pp = c('center', 'scale'),  k = 25)
  
  sd_knn <- getSDPrediction(select(df_cal_i,starts_with("NDVI")), df_cal_i$yield, select(df_val_i,starts_with("NDVI")), 
  ...
  write.csv(df_performance, file = file.path(dir_out, paste0("performance_", crop_i, "_", i,"_now.csv")))
  write.csv(df_cor, file = file.path(dir_out, paste0("cor_", crop_i, "_", i,"_now.csv")))
