...
for (crop_i in c("wheat")){ # crop_i = calibration data frames per crop
  ...
  outlist <- list()
  for (j in seq(100, 2000, by = 100)){
    print(paste("Training set size = ", j))
    ### CROSS VALIDATION i --------------
  ...
  write.csv(df_performance_all, file = file.path(dir_out, paste0("samplesize_performance_", crop_i, "_", i, "_", j,".csv")))
